...
/* Calcule la liste des blocs de base : il faut d�limiter les BB,
   en parcourant la liste des lignes/instructions � partir de la
   premiere, il faut s'arreter � chaque branchement (et prendre
   en compte le delayed slot qui appartient au meme BB, c'est
   l'instruction qui suit tout branchement) ou � chaque label
   (on estime que tout label est utilis� par un saut et donc
   correspond bien � une entete de BB). */
...
/* Calcule les successeurs et pr�d�cesseurs des BB.
   Pour cela, il faut commencer par les successeurs et it�rer sur tous
   les BB d'une fonction. Il faut determiner si un BB a un ou deux
   successeurs : d�pend de la pr�sence d'un saut � la fin ou non.
...
/* en implementant la fonction test de la classe BB, permet de tester des
   ...
   c'est le cas -- voir la classe Basic_block et la m�thode test
 */
void Function::test(){
	int size=(int)_myBB.size();
	for(int i=0; i<size; i++) {
